...
def login(username: str, password: str) -> str:
    user_data = next(
        (u for u in db_manager.read("users") if u["username"] == username), None
    )
    if not user_data or not verify_password(
        password, user_data["salt"], user_data["hashed_password"]
    ):
...
def _load_portfolio(user: User) -> Portfolio:
    ...
    user_portfolio_data = next(
        (p for p in portfolios_data if p.get("user_id") == user.user_id), None
    )
    wallets = {}
    if user_portfolio_data and "wallets" in user_portfolio_data:
        for code, data in user_portfolio_data["wallets"].items():
            try:
                wallets[code] = Wallet(
                    currency=get_currency(code), balance=data.get("balance", 0.0)
                )
...
def _save_portfolio(portfolio: Portfolio):
    portfolios_data = db_manager.read("portfolios")
    user_portfolio_data = next(
        (p for p in portfolios_data if p.get("user_id") == portfolio.user_id), None
    )
    serialized_wallets = {
        code: {"balance": w.balance} for code, w in portfolio.wallets.items()
    }
    if user_portfolio_data:
        user_portfolio_data["wallets"] = serialized_wallets
    else:
        portfolios_data.append(
            {"user_id": portfolio.user_id, "wallets": serialized_wallets}
        )
...
def show_portfolio(base: Optional[str] = None) -> str:
    ...
    for code, wallet in portfolio.wallets.items():
        rate = (
            1.0
            if code == base_code
            else rates.get(f"{code}_{base_code}", {}).get("rate")
        )
        value_str = (
            f"{wallet.balance * rate:.2f} {base_code}" if rate else "Курс не найден"
        )
        if rate:
...
def buy(currency: str, amount: float) -> str:
    ...
    try:
        ...
        raise InsufficientFundsError(
            code=base_currency_code, available=0.0, required=cost
        )

    base_wallet.withdraw(cost)

    target_wallet = (
        portfolio.get_wallet(currency_to_buy.code)
        if currency_to_buy.code in portfolio.wallets
        else portfolio.add_wallet(currency_to_buy.code)
    )
    ...
    return (
        f"✓ Покупка выполнена: {amount:.4f} {currency_to_buy.code}\n"
...
def get_rate(from_currency: str, to_currency: str) -> str:
    ...
    if not rate_info:
        raise RateNotFoundError(code=pair)
...
def show_rates(currency: Optional[str] = None, top: Optional[int] = None) -> str:
    ...
    return (
        f"Курсы из кеша (обновлено: {rates_data['last_refresh']}):\n"
        + table.get_string()
    )
